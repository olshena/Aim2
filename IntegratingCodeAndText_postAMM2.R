        ...
        for(j in 1:n.lambdas)
          {
            ...
            #print(final.fit.pruned$cptable)
...
#set.seed(12345)			    
#temp <- composite.rpart(dat=housing.data,n.grid=20,mult=5,outvar="mdev")
...
# If we ran a rpart tree on predicted values from RandomForest:

#```{r}
#fit.rf.dat <- randomForest(mdev ~ .,data = housing.data) 
#predict.rf.dat <- predict(fit.rf.dat,newdata=housing.data,
 #                         predict.all=FALSE)
#dat.complete2<-housing.data
#dat.complete2$mdev<-NULL
#dat.complete2$predict.rf.dat<-predict.rf.dat
#(justRpart<-rpart(predict.rf.dat ~ .,data = dat.complete2)  )
#justRpart$cptable
#min.CP<-justRpart$cptable[which(justRpart$cptable[,4]==min(justRpart$cptable[,4])),1][1]
#justRpartpruned<-prune(justRpart,cp=min.CP)
#prp(justRpartpruned)
#rpart.test.dat.predicted.fit.resub<-predict(object=justRpartpruned,newdata=dat.complete2)
#rpart.error.test.dat.resub <- sum((housing.data$mdev-rpart.test.dat.predicted.fit.resub)^2)
#rpart.error.test.dat.resub.RF <- sum((dat.complete2$predict.rf.dat-rpart.test.dat.predicted.fit.resub)^2)

#```

#The resubstitution estimate of prediction error for True outcome is `r  round(rpart.error.test.dat.resub,2)`.   
#The resubstitution estimate of prediction error for Predicted from RF outcome `r  round(rpart.error.test.dat.resub.RF,2)`. 
