...
best from an efficiency perspective; hence the choice of
...
The main function is the `aim2` function which has calls:  
  ...
  - **seed** fixes the random number generator for reproducibility  -- This should be outside
...
\item {\bf optimism} is $2*\sum^{n_1}_{i=1} C^*_i(\lambda)$ from the right hand side of sum in \eqref{optcor} 
