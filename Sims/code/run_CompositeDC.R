...
CompositeDC <- paste("CompositeDC/October2017/uplim40/",model,"_",tag,"_",sep="")
...
for (idx in 1:n.simulations)
{
  ...
  system.time(Model<-composite.rpart(dat=training.data.all,n.grid=200,uplim=40,outvar="training.y",alpha.Fixed=FALSE))
  ...
  write(ErrorToTruthCurrent,paste(CompositeDC,"CompositeDC_Train500_PRUNED_uplim_ErrorToTruthCurrent_",idx,".txt",sep=""),ncol=1)        
